...
abbrlink: 52088fd5
summary: >-
  这篇文章介绍了针对Minecraft服务器新手腐竹推荐的一些实用插件，内容基于作者个人使用体验和整合包增删调整。插件推荐包括连锁挖矿和砍树插件如Veinminer和TreeCuter，以简化前期资源收集；世界保护插件NoBuildPlus用于管理各种游戏内行为规则，防止破坏和PVP；玩家背包回滚插件InventoryRollbackPlus便于物品恢复；优化插件如Dmenu菜单、WorldListTrashCan清扫、Let
  Your Memory
  Rest内存优化、LogFilter日志过滤和ServerShutdown服务器关闭倒计时，提升服务器性能和用户体验；以及计分板插件Tab用于信息展示。这些插件旨在帮助新手腐竹轻松维护服务器，适应与家人和同学的小型游玩场景，基于Paper、Spigot等核心，版本1.21.8。
