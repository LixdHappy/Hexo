...
abbrlink: de7f0b92
summary: >- 
  这篇文章介绍了安知鱼主题中的多种标签插件功能，包括段落文本、行内文本、文本样式、分栏和按钮插件的使用方法。详细说明了每种插件的语法、参数选项和实际应用示例，如通过p标签实现彩色文字和超大号文字效果，span标签支持字体、颜色、大小和对齐调整，text标签提供下划线、着重号、波浪线等样式，tabs标签用于创建分栏内容并支持自定义图标和预设选择，btns标签则用于生成按钮布局，支持圆角、圆形样式和网格排列。这些插件增强了博客内容的视觉表现和交互性，适用于Hexo博客框架下的个性化内容展示。
date: 2025-08-24 12:20:44
comments:
top: 100
---
